...
.. code-block:: c++
    ...
    if (condition) // if后接一个空格，圆括号里没有空格.
    {  
      ...  // 2 空格缩进.
    } 
    else if (...) // else 与 if 的另起一行.
    {  
      ...
    } 
    else
...
任何情况下条件语句都必须单独一行，哪怕只有一行语句:

.. code-block:: c++

    if (x == kBar) return new Bar();  // 差
    if (x == kFoo)                    // 好				
    {
    	return new Foo();
    }
...
在单语句循环里, 无论语句是否简单，必须保留括号：

.. code-block:: c++

    for (int i = 0; i < kSomeNumber; ++i)		// 差
      printf("I love you\n");

    for (int i = 0; i < kSomeNumber; ++i) 	// 好
    {
...
注意, 上例的逻辑与 (``&&``) 操作符均位于行头. 可以考虑额外插入圆括号, 合理使用的话对增强可读性是很有帮助的. 此外, 直接用符号形式的操作符, 比如 ``&&`` 和 ``~``, 不要用词语形式的 ``and`` 和 ``compl``.
...
访问控制块的声明依次序是 ``public:``, ``protected:``, ``private:``, 每个都缩进 1 个空格, 所有类声明必须写在.h文件中, 实现必须写在.cpp文件中.
...
.. code-block:: c++
    ...
    class MyClass : public OtherClass 
    {
     public:      // 注意有一个空格的缩进
      ...
      void SetSomeVar(int nVar);
      int SomeVar() const;

     private:
      bool IsSomeInternalFunction();
...
.. code-block:: c++
    ...
    // 如果所有变量能放在同一行:
    MyClass::MyClass(int nVar) : m_nSomeVar(nVar) 
    {
      DoSomething();
    }
    ...
    // 如果不能放在同一行,
    // 必须置于冒号后, 并缩进 4 个空格
    MyClass::MyClass(int nVar)
        : m_nSomeVar(nVar), m_nSomeOtherVar_(nVar + 1) 
    {
      DoSomething();
    }
    ...
    // 并逐行对齐
    MyClass::MyClass(int nVar)
        : m_nSomeVar(nVar),             // 4 space indent
          m_nSomeOtherVar_(nVar + 1) 
    {
      DoSomething();
    }
...
不允许随意的留白，例如两个函数定义之间的空行超过 1 行, 函数体首尾不要留空行, 任意处留白不得超过1行. 但是为了代码美观，在代码行较多的函数中，可以在两块相对独立的逻辑间用空行进行隔离。
